...
@flask_app.route('/')
def home(): return "v73 FINAL LIVE"
...
def deposit_choice_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton("💎 LTC", callback_data="dep_ltc"), InlineKeyboardButton("💜 SOL", callback_data="dep_sol")],[InlineKeyboardButton("🔙 Back", callback_data="profile")]])
def admin_kb():
    cfg = get_cfg(); prods = load_file("products.json")
    stock = len([x for x in prods.values() if not x.get("sold")]); sold = len([x for x in prods.values() if x.get("sold")])
...
def vendor_dash_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton("➕ Add Giftcard", callback_data="add_gift")],[InlineKeyboardButton("📦 My Stock", callback_data="my_stock"), InlineKeyboardButton("📈 My Sales", callback_data="my_sales")],[InlineKeyboardButton("💰 Earnings", callback_data="my_earn"), InlineKeyboardButton("🔙 Back", callback_data="main")]])
def vendor_buy_kb(bal):
    return InlineKeyboardMarkup([[InlineKeyboardButton(f"👑 Buy Vendor {VENDOR_PRICE}$", callback_data="buy_vendor")],[InlineKeyboardButton("🏛️ Deposit Funds", callback_data="dep_funds")],[InlineKeyboardButton("🔙 Back", callback_data="main")]])
def mark_kb(g,p,reg,price,orig):
    # FINAL: Orig button nai, middle e 39% ON
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("➖", callback_data="price_minus"), InlineKeyboardButton(f"📈 39% ✅ ON", callback_data="mark_39"), InlineKeyboardButton("➕", callback_data="price_plus")],
...
async def post_to_stock_channel(context, product, pid):
    try:
        code = product['code']; masked = f"{code[:4]}...."; price = product['price']; avl = product.get('orig', '?')
        g = '✅' if product.get('g') else '❌'; p = '✅' if product.get('p') else '❌'; reg = 'REG ✅' if product.get('reg') else 'UNREG 🌐'
...
async def set_cmds(app):
    await app.bot.set_my_commands([BotCommand("start","🚀 Launch"),BotCommand("latest","📋 Latest"),BotCommand("stock","📦 Stock"),BotCommand("deposit","💰 Deposit"),BotCommand("support","💬 Support"),BotCommand("profile","👤 Profile"),BotCommand("admin","👑 Admin"),BotCommand("vendor","👑 Vendor")])
...
async def latest_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(f"{p['code'][:12]} ${p['price']}", callback_data=f"view_{pid}")] for pid,p in active[-10:][::-1]]; kb.append([InlineKeyboardButton("🔙 Back", callback_data="main")])
...
async def profile_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id; u = get_user(uid).get(str(uid), {}); bal = u.get('balance',0)
...
async def msg_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if wait and wait.startswith("deposit_txid_"):
        coin = wait.replace("deposit_txid_",""); did = str(uuid.uuid4())[:6]; deposits = load_file("deposits.json"); deposits[did] = {"user_id": uid, "coin": coin, "txid": txt, "status": "pending"}; save_file("deposits.json", deposits)
        await update.message.reply_text(f"✅ Submitted {coin} {did}", reply_markup=profile_menu(uid, load_file("users.json").get(str(uid),{}).get("balance",0)))
        try:
    ...
    if wait == "add_stock":
        amt = get_amount(txt)
        if amt == 0: await update.message.reply_text("❌ avl pai nai! Example: 4511... $25"); return
    ...
    if wait == "set_price_custom":
        ...
        except: await update.message.reply_text("Price dao e.g. 0.24"); return
        ...
        await update.message.reply_text(f"💳 {context.user_data.get('pending_code','')[:4]}.... avl ${context.user_data.get('pending_amt')}", reply_markup=mark_kb(g,p,reg,price,context.user_data.get('pending_amt'))); return
    if wait == "set_perc_custom":
        ...
        except: await update.message.reply_text("Number dao e.g. 39", reply_markup=admin_kb())
        context.user_data["wait"]=None; return
    if wait == "add_bal":
        try:
            ...
            users[uid_t]["balance"]=users[uid_t].get("balance",0)+amt; save_file("users.json", users); await update.message.reply_text(f"✅ Added ${amt} to {uid_t}", reply_markup=admin_kb())
...
async def cb_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer(); d=q.data; uid=q.from_user.id; users=get_user(uid); u=users.get(str(uid),{}); bal=u.get("balance",0); cfg=get_cfg()
    if d=="main": stock=len([x for x in load_file("products.json").values() if not x.get("sold")]); await q.edit_message_text(f"🎁 PREPAIDS GIFT'S\n💰 Bal: ${bal}\n📦 Stock: {stock}\n📈 {cfg['perc']}%", reply_markup=top_menu(uid==ADMIN_ID)); return
    if d=="profile": await q.edit_message_text(f"🪪 PROFILE\nID: {uid}\n💰 Balance: ${bal}\n👑 Vendor: {'✅' if u.get('is_vendor') else '❌'}", reply_markup=profile_menu(uid, bal)); return
    ...
    if d=="dep_ltc": qr=f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={LTC_ADDRESS}"; txt=f"💎 LTC Deposit\nAddress:\n`{LTC_ADDRESS}`\nMin $5"; kb=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Submit TXID", callback_data="submit_ltc")],[InlineKeyboardButton("🔙 Back", callback_data="dep_funds")]]);
        try: await q.edit_message_text(txt, reply_markup=kb, parse_mode="Markdown"); await context.bot.send_photo(chat_id=uid, photo=qr)
        except: await q.edit_message_text(txt, reply_markup=kb, parse_mode="Markdown"); return
    if d=="dep_sol": qr=f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={SOL_ADDRESS}"; txt=f"💜 SOL Deposit\nAddress:\n`{SOL_ADDRESS}`\nMin $5"; kb=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Submit TXID", callback_data="submit_sol")],[InlineKeyboardButton("🔙 Back", callback_data="dep_funds")]]);
        try: await q.edit_message_text(txt, reply_markup=kb, parse_mode="Markdown"); await context.bot.send_photo(chat_id=uid, photo=qr)
        except: await q.edit_message_text(txt, reply_markup=kb, parse_mode="Markdown"); return
    ...
    if d=="list":
        ...
        kb=[[InlineKeyboardButton(f"{p['code'][:12]} ${p['price']}", callback_data=f"view_{pid}")] for pid,p in active[-15:][::-1]]; kb.append([InlineKeyboardButton("🔙 Back", callback_data="main")])
    ...
    if d=="vendor_panel":
        if u.get("is_vendor"): prods=load_file("products.json"); my_stock=len([p for p in prods.values() if p.get("owner")==uid and not p.get("sold")]); my_sold=len([p for p in prods.values() if p.get("owner")==uid and p.get("sold")]); await q.edit_message_text(f"⚜️ SELLER'S DASHBOARD 🪝\n📦 Stock: {my_stock}\n✅ Sold: {my_sold}\n💰 Earn: ${u.get('earn',0)}\n💳 Bal: ${bal}", reply_markup=vendor_dash_kb())
        else: await q.edit_message_text(f"⚜️ SELLER'S DASHBOARD\nBuy Vendor ${VENDOR_PRICE} Bal ${bal}", reply_markup=vendor_buy_kb(bal)); return
    if d=="buy_vendor":
        ...
        users[str(uid)]["balance"]=bal-VENDOR_PRICE; users[str(uid)]["is_vendor"]=True; save_file("users.json", users); await q.edit_message_text("✅ BOUGHT!", reply_markup=vendor_dash_kb()); return
    if d=="my_stock": prods=load_file("products.json"); my=[(k,v) for k,v in prods.items() if v.get("owner")==uid and not v.get("sold")]; msg=f"📦 My Stock {len(my)}\n"; kb=[];
        for pid,p in my[-10:]: msg+=f"{p['code'][:12]} ${p['price']}\n"; kb.append([InlineKeyboardButton(f"🗑️ {p['code'][:6]}", callback_data=f"del_{pid}")])
        if not my: msg="No stock"; kb.append([InlineKeyboardButton("➕ Add", callback_data="add_gift"), InlineKeyboardButton("🔙 Back", callback_data="vendor_panel")]); await q.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(kb)); return
    if d.startswith("del_"): pid=d.replace("del_",""); prods=load_file("products.json");
        if pid in prods and prods[pid].get("owner")==uid: del prods[pid]; save_file("products.json", prods); await q.edit_message_text(f"Deleted {pid}", reply_markup=vendor_dash_kb()); return
    if d=="my_sales": prods=load_file("products.json"); sold=[(k,v) for k,v in prods.items() if v.get("owner")==uid and v.get("sold")]; msg=f"📈 Sales {len(sold)}\n";
        for pid,p in sold[-10:]: msg+=f"{p['code'][:10]} ${p['price']}\n";
        if not sold: msg="No sales"; await q.edit_message_text(msg, reply_markup=vendor_dash_kb()); return
    if d=="my_earn": await q.edit_message_text(f"💰 Earn ${u.get('earn',0)}", reply_markup=vendor_dash_kb()); return
    if d=="add_gift": context.user_data["wait"]="add_stock"; await q.edit_message_text("📝 SEND: `4511... $25` last $ = avl", parse_mode="Markdown", reply_markup=vendor_dash_kb() if u.get("is_vendor") else admin_kb()); return
    if d=="set_perc": context.user_data["wait"]="set_perc_custom"; await q.edit_message_text(f"Current {cfg['perc']}% \nSend new % e.g. 39"); return
    if d=="add_bal": context.user_data["wait"]="add_bal"; await q.edit_message_text("Format: USERID AMOUNT", reply_markup=admin_kb()); return
    if d=="users_list": users=load_file("users.json"); msg=f"👥 Users {len(users)}\n";
    ...
    if d=="pending_dep": deposits=load_file("deposits.json"); msg="💳 PENDING DEP\n"; kb=[];
        ...
        if not kb: msg="No pending"; kb.append([InlineKeyboardButton("🔙 Back", callback_data="admin")]); await q.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(kb)); return
    if d=="pending_orders": orders=load_file("orders.json"); msg=f"🛒 PENDING ORDERS\n"; kb=[];
        for oid,o in list(orders.items())[-15:][::-1]:
            if "pending" in o.get("status",""): msg+=f"{oid} Buyer:{o['buyer_id']} ${o['price']}\n"; kb.append([InlineKeyboardButton(f"✅ {oid}", callback_data=f"approv
