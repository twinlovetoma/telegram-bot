...
def mark_kb(g,p,reg,price,amt,cat="Giftcard"):
    txt_g = "G ON" if g else "G OFF"
    txt_p = "P ON" if p else "P OFF"
    txt_r = "REG" if reg else "UNREG"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(txt_g, callback_data="mark_g"), InlineKeyboardButton(txt_p, callback_data="mark_p"), InlineKeyboardButton(txt_r, callback_data="mark_reg")],
...
async def deposit_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"DEPOSIT LTC {LTC_ADDRESS} SOL {SOL_ADDRESS}", reply_markup=top_menu(update.effective_user.id==ADMIN_ID))
...
async def cb_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d=="top_list":
        ...
        return
    if d=="top_dep":
        await q.edit_message_text(f"DEPOSIT LTC {LTC_ADDRESS} SOL {SOL_ADDRESS}", reply_markup=top_menu(uid==ADMIN_ID))
        return
    if d=="top_profile":
    ...
    if d in ["mark_g","mark_p","mark_reg","price_minus","price_plus"]:
        ...
        await q.edit_message_text(f"Card {code[:20]} ${price}", reply_markup=mark_kb(g,p,reg,price,amt,cat))
        return
    ...
    if d=="mark_save":
        ...
        reg=context.user_data.get('mark_reg',False)
        price=context.user_data['pending_price']
        codes=context.user_data.get('pending_codes',[context.user_data['pending_code']])
        cat=context.user_data.get('upload_type',"Giftcard")
        prods=load_file("products.json")
        for code in codes:
            m=re.search(r'USD\$?\s*(\d+(?:\.\d+)?)', code, 
